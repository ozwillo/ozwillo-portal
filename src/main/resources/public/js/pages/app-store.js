...
export default class AppStore extends React.Component {
  ...
  _askForFilters = async () => {
    ...
    const askFilterPermission = JSON.parse(
      localStorage.getItem("askFilterPermission")
    );
    ...
    if (potentialOldFilters && !askFilterPermission) {
      ...
      potentialOldFilters &&
      askFilterPermission &&
      numberPotentialActiveFilters > 0
    ) {
      swal({
  ...
  render() {
    ...
    const filterCounter = activeFiltersNumber > 0 && (
      <div className={"badge-filter-close"}>
        <CustomTooltip title={i18n._(t`active-filters`)}>
          {activeFiltersNumber}
        </CustomTooltip>
      </div>
    );
    const filterCounterHeader = activeFiltersNumber > 0 && (
      <React.Fragment>
        <CustomTooltip title={i18n._(t`reset-filters`)}>
          <i
            className={"reset-filters fa fa-trash"}
      ...
      <React.Fragment>
        ...
        {loading ? (
          ...
          <div className={"app-store-wrapper oz-body"}>
            <SideNav
              isCloseChildren={filterCounter}
              isOpenHeader={filterCounterHeader}
            >
              <SearchAppForm
                ref={"searchAppForm"}
