...
export default class AppInstall extends React.Component {
  ...
  _loadOrgs = async () => {
    ...
    const newOrganizationsAvailable = await this._disableOrganizationWhereAppAlreadyInstalled(
      data
    );
    this.setState({
  ...
  _disableOrganizationWhereAppAlreadyInstalled = async (allOrganizations) => {
    const { app } = this.state;
    const organizationUnavailable = await unavailableOrganizationToInstallAnApp(
      app.id,
      app.type
    );
  ...
  render() {
    ...
    return (
      <div className={"app-install-wrapper"}>
        ...
        <div className={"flex-row header-app-install"}>
          <div className={"information-app flex-row"}>
            ...
            <div className={"information-app-details"}>
              <div className="information-app-name">
                <strong>{app.name}</strong>
              </div>
              <div className="information-app-provider">{app.provider}</div>
              <div className="information-app-description">
                {app.description}
                &nbsp;
        ...
        <div
          className={"flex-row app-install-description"}
          ref={(ref) => (this.longDescription = ref)}
        >
...
export class InstallForm extends React.Component {
  ...
  _installButtonIsDisabled = () => {
    ...
    if (
      !installType ||
      buying ||
      (organizationSelected && organizationSelected.disabled)
    ) {
      return true;
    } else if (
      installType &&
      ...
      <React.Fragment>
        ...
        !(app.type === "service") ? (
          <div className={"install-selector"}>
            <label
              className={
                organizationsAvailable.loading ? "label-spinner" : null
              }
            >
              <Trans>For which organization</Trans>
            </label>
            <div className={"select-organization"}>
              <Spinner
                display={organizationsAvailable.loading}
                className={"organization-spinner"}
              />
              <Select
                disabled={
                  disabledOrganization || organizationsAvailable.loading
                }
                className={"select"}
        ...
        <div className={"flex-row install-area"}>
          ...
          {installed ? (
            <div className="install installed">
              <Link
                className="btn btn-default-inverse pull-right btn-install"
                to={`/${this.props.config.language}/store`}
              >
                {i18n._("ui.appstore")}
              </Link>
              <Link
                className="btn btn-default-inverse pull-right btn-install dashboard"
                to={"/my/dashboard"}
              >
                {i18n._("my.dashboard")}
              </Link>
            ...
            <div className="install">
              <button
                className="btn pull-right btn-install"
                disabled={this._installButtonIsDisabled()}
                onClick={this._doInstallApp}
              >
                {i18n._("store.install")}
              </button>
            </div>
