    ...
    render: function() {
        ...
        return (
            <div className={className} style={{ marginTop: '1em' }}>
                <label htmlFor="usersEmail" className="control-label col-sm-2">{this.context.t('settings-invite-by-email')}</label>
                <div className="col-sm-7">
                    <textarea id="usersEmail" className="form-control" cols="30" rows="5"
                        title={this.context.t('settings-invite-by-email-title')} placeholder={this.context.t('settings-invite-by-email-title')}
                        value={this.state.userInput} onChange={this.updateUserInput} />
                </div>
                <div className="col-sm-2">
                    <button type="submit" className="btn oz-btn-save" onClick={this.handleAddUsers}>{this.context.t('ui.add')}</button>
                </div>
            </div>
        )
    }
