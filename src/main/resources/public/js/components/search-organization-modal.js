...
SearchOrganizationModal.contextTypes = {
...
var SearchOrganizationForm = createClass({
    ...
    renderGeneralErrorMessage: function() {
        if (this.state.errors.indexOf('general') != -1) {
            return (
                <div className="alert alert-danger">{this.context.t('search.organization.cannot-be-used')}</div>
            )
        } else if (this.state.errors.indexOf('technical') != -1) {
            return (
                <div className="alert alert-danger">{this.context.t('search.organization.technical-problem')}</div>
            )
        }
    },
        ...
        if (!this.state.exist_in_dc && this.state.country) {
            return (
                <button type="submit" className="btn btn-default-inverse"
                        onClick={this.searchOrganization}>{this.context.t('ui.search')}</button>
            )
        } else if (this.state.exist_in_dc && !this.state.exist_in_kernel) {
            return (
                <button type="submit" className="btn btn-default-inverse"
                        onClick={this.onNextStep}>{this.context.t('ui.next')}</button>
            )
        }
    },
    render: function () {
            ...
            <div>
                ...
                <div className="modal-footer">
                    <button type="button" key="cancel" className="btn oz-btn-cancel"
                            onClick={this.props.cancelHandler}>{this.context.t('ui.cancel')}</button>
...
SearchOrganizationForm.contextTypes = {
    t: PropTypes.func.isRequired
};
        ...
        return (
            <div className="form-group">
                <label htmlFor="country" className="col-sm-3 control-label required">
                    {this.context.t('search.organization.country')} *
                </label>
            ...
            return (
                <div className={formGroupClass}>
                    <label htmlFor="legal_name" className="col-sm-3 control-label required">
                        {this.context.t('search.organization.legal-name')} *
                    </label>
...
var SectorType = createClass({
   render: function() {
       if (this.props.display) {
           ...
           return (
               <div className={formGroupClass}>
                   <label className="col-sm-3 control-label required">{this.context.t('search.organization.sector-type')} * </label>
                   <div className="col-sm-8">
...
SectorType.contextTypes = {
    t: PropTypes.func.isRequired
};

export default SearchOrganizationModal;
