...
var Instance = createClass({
    ...
    render: function () {
        ...
        if (applicationInstanceStatus === 'STOPPED') {
            ...
            buttons.push(
                <button key={this.props.id + '-untrash'} type="button" className="btn oz-btn-danger pull-right" onClick={this.confirmUntrash}>{t('ui.cancel')}</button>
            );
            buttons.push(
            ...
            dialogs.push(
                <Modal key={this.props.id + '-untrashModal'} ref="confirmUntrash" title={confirmUntrashTitle} successHandler={this.untrash} buttonLabels={{ 'cancel': t('ui.cancel'), 'save': t('ui.confirm') }} >
...
ReactDOM.render( <MyApps />, document.getElementById("myapps") );
