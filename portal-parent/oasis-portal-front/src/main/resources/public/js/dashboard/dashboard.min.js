!function () {
    var Dashboard = React.createClass({
        displayName: "Dashboard", componentDidMount: function () {
            $.ajax({
                url: dash_service + "/dashboards", type: "get", dataType: "json", success: function (data) {
            ...
            return function (app) {
                $.ajax({
                    url: dash_service + "/apps/move/" + app.id + "/to/" + dash.id, type: "post", success: function () {
                    }, error: function (xhr, status, err) {
                        this.setState(this.getInitialState()), this.componentDidMount()
                    }.bind(this)
                });
                {
                    var state = this.state, index = this.findById(state.apps, app), dashIdx = this.findById(state.dashboards, dash);
                    state.apps.splice(index, 1)
                }
                state.dashboards[dashIdx].apps.push(app), state.dragging = !1, this.setState(state)
            }.bind(this)
