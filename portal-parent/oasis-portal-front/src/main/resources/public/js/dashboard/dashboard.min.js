!function () {
    var Dashboard = React.createClass({
        ...
        componentDidMount: function () {
            this.initNotificationsCheck(), $.ajax({
                ...
                url: dash_service + "/pending-apps", type: "get", dataType: "json", success: function (data) {
                    this.state.pendingApps = data, this.setState(this.state)
                }.bind(this), error: function (xhr, status, err) {
                }.bind(this)
            })
        },
        ...
        switchToDashboard: function (dash) {
            return function () {
                var state = this.state;
                state.dash = dash, state.loadingApps = !0, state.pendingApps = null, this.setState(state), $.ajax({
                    ...
                    dataType: "json",
                    success: function (data) {
                        state.pendingApps = data, this.setState(state)
                    }.bind(this),
                    error: function (xhr, status, err) {
                    }.bind(this)
                })
        ...
        displayName: "Desktop", getInitialState: function () {
            ...
            if (this.props.pendingApps && this.props.dash.main)for (var i in this.props.pendingApps) {
                var app = this.props.pendingApps[i];
                icons.push(PendingApp({key: app.id, app: app}))
            }
    ...
    }), PendingApp = React.createClass({
        displayName: "PendingApp", render: function () {
            return React.DOM.div({className: "appzone"}, React.DOM.div({className: "dropzone"}), React.DOM.div({
                ...
                draggable: "false",
                className: "pending"
            }), React.DOM.p(null, this.props.app.name)))
        }
